...
const LinkAccount = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setError(""); setOkMsg(""); setFieldErrors({});
    if (!accountNumber || !bankName || !accountName) {
      setError("Please fill all fields."); return;
    }

    const token = localStorage.getItem("access_token");
    if (!token) { setError("Please log in first."); navigate("/login"); return; }
    ...
    try {
      ...
      const status = err?.response?.status;
      const resp = err?.response?.data;
      if (!err?.response) setError("Network/CORS error. Ensure API allows your origin and 'Authorization' header.");
      else if (status === 401) { setError("Your session expired or token is invalid. Please login again."); localStorage.removeItem("access_token"); navigate("/login"); }
      else if (status === 422 && resp?.errors) { setFieldErrors(resp.errors); setError(resp.message || "Validation error."); }
      else setError(resp?.message || resp?.error || "Link account failed. Please try again.");
    ...
    <div className="flex min-h-screen w-full">
      ...
      <div className="flex-1">
        ...
        <div className="bg-[#F5F7FF] min-h-screen w-full p-5 sm:p-10 pb-28 sm:pb-10">
          {/* Desktop title */}
          <h1 className="hidden sm:block text-[26px] font-medium tracking-tight">Link Account</h1>
          <p className="hidden sm:block text-lg text-gray-500 pb-6">Add your bank details</p>
            ...
            <div className="md:hidden mt-4">
              <div className="bg-amber-50 text-amber-700 border border-dashed border-amber-400 rounded-xl p-4">
                <p className="text-center text-sm">
                  Kindly note that a service charge of <span className="font-semibold">N50</span> will be deducted
                </p>
              </div>
            </div>
          </div>
