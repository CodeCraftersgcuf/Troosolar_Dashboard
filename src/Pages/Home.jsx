          ...
          <div className="bg-[#F5F7FF] p-5">
            ...
            <div className="flex justify-between items-center">
              <div className="flex justify-start gap-2 items-center">
                ...
                <div>
                  <h1 className="text-2xl">
                    Hi, <span className="sm:text-[#273e8e] text-black">{firstName}</span>
                  </h1>
                  <p className="mt-1">Welcome to your dashboard</p>
                </div>
              </div>
            ...
            <div className="grid items-center md:grid-cols-2 grid-cols-1 xl:grid-cols-3 gap-5 mt-4">
              <div className="flex justify-start items-center sm:hidden gap-3 pt-2">
                <button
                  onClick={() => setShowWallet(!showWallet)}
                  className={`border text-sm w-[40%] border-[#273e8e] py-4 rounded-full ${
                    showWallet ? "text-white bg-[#273e8e]" : "text-[#273e8e] bg-white"
                  }`}
                >
                  Loan Wallet
                </button>
                <button
                  onClick={() => setShowWallet(!showWallet)}
                  className={`border text-sm w-[40%] border-[#273e8e] py-4 rounded-full ${
                    showWallet ? "text-[#273e8e] bg-white" : "text-white bg-[#273e8e]"
                  }`}
                >
                  Shopping Wallet
                </button>
              ...
              <div className="block sm:hidden">{showWallet ? <LoanWallet /> : <ShoppingWallet />}</div>
              <div className="sm:block hidden">
                <LoanWallet />
              </div>
              <div className="sm:block hidden">
            ...
            <div className="mt-6">
              <h1 className="text-xl sm:block hidden font-[500]">Solar Bundles</h1>
              <p className="text-gray-500 pt-4 text-sm w-1/2 sm:block hidden sm:text-base">
                You can select from one of our custom bundles tailored towards
                various homes and uses, or you can build your custom builder from
                custom builder
            ...
            <div className="my-4 hidden sm:flex gap-5 overflow-x-auto pb-4 scrollbar-hide">
              {bundlesErr && <div className="text-red-600 text-sm">{bundlesErr}</div>}
              {bundlesLoading && <div className="text-gray-600 text-sm">Loading…</div>}
              {!bundlesLoading &&
                !bundlesErr &&
            ...
            <div className="my-4 sm:hidden">
              ...
              {prodError && <p className="text-red-600 text-sm mt-3">{prodError}</p>}
              {prodLoading && <p className="text-gray-600 text-sm mt-3">Loading…</p>}
