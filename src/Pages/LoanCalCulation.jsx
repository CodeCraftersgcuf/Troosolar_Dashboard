...
const LoanCalculation = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const payload = { product_amount: pAmt, loan_amount: lAmt, repayment_duration: repaymentMonths };
      const { data } = await axios.post(API.LOAN_CALCULATION, payload, {
    ...
    <div className="flex min-h-screen w-full overflow-clip">
      ...
      <div className="flex-1">
        ...
        <div className="bg-[#F5F7FF] min-h-screen w-full p-5 sm:p-10 pb-28 sm:pb-10">
          {/* Desktop title */}
          <h1 className="hidden sm:block text-[26px] font-medium tracking-tight">Loan Calculation</h1>

          <div className="flex flex-col md:flex-row gap-6">
            {/* Left: form */}
            <form id="loan-calc-form" onSubmit={handleSubmit} className="w-full md:w-1/2 rounded-lg">
              <Input
                id="productAmount"
            ...
            <div className="w-full md:w-1/2 flex flex-col items-start justify-start gap-4">
              ...
              <div className="bg-amber-50 text-amber-700 border border-dashed border-amber-400 rounded-xl p-4 w-full">
                <p className="text-sm leading-relaxed">
                  ...
                  {pAmt > 0 && lAmt > 0 && (
                    <>
                      {" "}complete the 30% payment by funding your wallet directly with{" "}
                      <span className="font-semibold">{formatNGN(Math.max(pAmt - maxLoan, 0))}</span>
                    </>
                  )}
                </p>
              </div>
        ...
        <div className="sm:hidden fixed left-0 right-0 bottom-24 px-5">
          ...
          >
            {result ? "Proceed" : loading ? "Calculating..." : "Calculate"}
          </button>
        </div>
      </div>
    </div>
